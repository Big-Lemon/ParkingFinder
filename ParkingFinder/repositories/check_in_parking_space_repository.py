...
from ParkingFinder.mappers.parking_space_mapper import CheckInParkingSpaceMapper
from ParkingFinder.tables.check_in_parking_space import CheckInParkingSpace


class CheckInParkingSpaceRepository(object):
    ...
    def read_one(user_id):
        ...
        with create_session() as session:
            parking_space = session.query(CheckInParkingSpace).filter(
                CheckInParkingSpace.user_id == user_id).one()
            entity = CheckInParkingSpaceMapper.to_entity(record=parking_space)
            raise Return(entity)
    ...
    def remove(user_id):
        ...
        """
        with create_session() as session:
            rows = session.query(CheckInParkingSpace).filter(
                CheckInParkingSpace.user_id == user_id
            ).delete()
            if rows == 0:
                raise NoResultFound
    ...
    def _update(cls,
        ...
        """
        with create_session() as session:
            _parking_space = session.query(CheckInParkingSpace).filter(
                CheckInParkingSpace.user_id == parking_space.user_id
            ).one()
            ...
            entity = CheckInParkingSpaceMapper.to_entity(record=_parking_space)
    ...
    def _insert(cls, parking_space):
        ...
        """
        with create_session() as session:
            _parking_space = CheckInParkingSpaceMapper.to_model(parking_space)
